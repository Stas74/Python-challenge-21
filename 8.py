...
# Pandas can be used when working with large datasets, or when performing data cleaning, wrangling, and analysis.
...
# Because Pandas exists externally to basic Python, we need to import it. 
# When importing, we give an alias to Pandas so it shortens our code when calling on functions from Pandas. 
...
# The first Pandas function we'll use is called using pd.read_csv(). 
# This function reads the path to a .csv file and stores it in a Pandas DataFrame. 
# A DataFrame in Pandas is a representation of data in a table, similar to what we typically see when working within Excel.

# df = pd.read_csv('path/to/csv/sample.csv')

# The second function we'll use is df.head(), which by default displays the top 5 rows of a dataframe. 
# However, we can change the number of rows it displays by inputting an integer value between the parentheses after .head with the number we would like to observe.

# df.head(21)

# The third function we'll use is df.tail(), which by default displays the bottom five rows of a dataframe. 
# However, we can change the number of rows it displays by inputting an integer value between the parentheses after .tail with the number we would like to observe.
...
# df['column_name'].max()
# The df.sort_values() allows us to reorder our dataframe in an ascending or descending order 
# given a column for pandas to work from. This is similar to the excel sort function.
